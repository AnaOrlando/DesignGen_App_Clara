...
mod_ibd_ui <- function(id){
  ...
  tagList(
    fluidRow(style = "height:8000px",
             box(width = 12, title = "Incomplete block design", 
...
#' 
#' @noRd 
mod_ibd_server <- function(input, output, session){
  ns <- session$ns
  # download input
    ...
    withProgress(message = 'Building graphic', value = 0, {
      ...
      if(is.null(input$data_assum) == F){
        if(!all(c("local", "block", "gen") %in% colnames(dat)))
          stop(safeError("Incomplete block design should have columns 'local', 'block' and 'gen'."))
